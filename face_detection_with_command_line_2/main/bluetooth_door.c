...
static const esp_gatts_attr_db_t door_controller_gatt_db[DCS_IDX_NB] =
{
    ...
    [DCS_IDX_VAL]               =
    {{ESP_GATT_AUTO_RSP}, {ESP_UUID_LEN_16, (uint8_t *)&door_controller_char_uuid, ESP_GATT_PERM_READ | ESP_GATT_PERM_WRITE,
      ...
      esp_ble_gap_start_advertising(&door_controller_adv_params);
		...
		case ESP_GATTS_WRITE_EVT:
			...
			// if(connection_id == param->write.conn_id && param->write.handle == door_controller_attribute_handle){
				// esp_ble_gatts_set_attr_value(door_controller_attribute_handle, param->write.len, param->write.value);
			// }
			break;
